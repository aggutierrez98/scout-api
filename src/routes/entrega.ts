...
export default function createEntregaRouter(entregaService: EntregaService) {
	...
	router.get("/", validate(GetEntregasSchema), entregaController.getItems);
	router.get(
		"/:id",
	...
	router.post("/", validate(PostEntregaSchema), entregaController.insertItem);
	router.post(
		"/bulk",
		validate(PostManyEntregasSchema),
		entregaController.insertManyItems,
	);
	router.put(
		"/:id",
