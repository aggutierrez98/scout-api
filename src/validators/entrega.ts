...
export const PostEntregaSchema = z
	.object({
		body: EntregaSchema,
	})
	.refine(async ({ body }) => {
...
export const PostManyEntregasSchema = z
	.object({
		body: z.object({
			entregas: z.array(EntregaSchema).min(1),
		}),
	})
	.superRefine(async ({ body }, ctx) => {
		...
		for (const entrega of body.entregas) {
			const key = `${entrega.scoutId}-${entrega.tipoEntrega}`;
			if (seen.has(key)) {
				ctx.addIssue({
					code: z.ZodIssueCode.custom,
					message:
						"La solicitud contiene entregas duplicadas (scoutId + tipoEntrega)",
					path: ["body", "entregas"],
				});
				return;
			}
		...
		const respItem = await EntregaRealizaModel.findFirst({
			where: {
				OR: body.entregas.map(({ scoutId, tipoEntrega }) => ({
					scoutId,
					tipoEntrega,
				})),
			},
		...
		if (respItem) {
			ctx.addIssue({
				code: z.ZodIssueCode.custom,
				message: "Algunas entregas ya estan registradas dentro del sistema",
				path: ["body", "entregas"],
			});
		}
	});

export const PutEntregaSchema = z
	.object({
		params: z.object({
			id: IdSchema.refine(validEntregaId),
		}),
